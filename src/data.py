...
def extract_attributes(line, corpus_vocab, attribute_vocab, use_ngrams=False):
...
def read_nmt_data(src, config, tgt, attribute_vocab, train_src=None, train_tgt=None,
    ...
    src_lines, src_content, src_attribute = list(zip(
        *[extract_attributes(line, config['data']['src_vocab'], pre_attr, pre_attr) for line in src_lines]
    ))
    ...
    tgt_lines, tgt_content, tgt_attribute = list(zip(
        *[extract_attributes(line, config['data']['tgt_vocab'], post_attr, post_attr) for line in tgt_lines]
    ))
